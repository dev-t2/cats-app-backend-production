"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Cat = void 0;
const swagger_1 = require("@nestjs/swagger");
const class_validator_1 = require("class-validator");
class Cat {
}
__decorate([
    (0, swagger_1.ApiProperty)({ required: true, description: '아이디' }),
    (0, class_validator_1.IsPositive)(),
    __metadata("design:type", Number)
], Cat.prototype, "id", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ required: true, description: '이메일' }),
    (0, class_validator_1.IsEmail)(),
    (0, class_validator_1.IsNotEmpty)(),
    __metadata("design:type", String)
], Cat.prototype, "email", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ required: true, description: '닉네임' }),
    (0, class_validator_1.IsString)(),
    (0, class_validator_1.IsNotEmpty)(),
    __metadata("design:type", String)
], Cat.prototype, "nickname", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ required: true, description: '비밀번호' }),
    (0, class_validator_1.IsString)(),
    (0, class_validator_1.IsNotEmpty)(),
    __metadata("design:type", String)
], Cat.prototype, "password", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: '아바타' }),
    (0, class_validator_1.IsString)(),
    __metadata("design:type", String)
], Cat.prototype, "avatar", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: '생성된 시간' }),
    (0, class_validator_1.IsDate)(),
    __metadata("design:type", Date)
], Cat.prototype, "createdAt", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: '업데이트된 시간' }),
    (0, class_validator_1.IsDate)(),
    __metadata("design:type", Date)
], Cat.prototype, "updatedAt", void 0);
exports.Cat = Cat;
//# sourceMappingURL=cat.entity.js.map